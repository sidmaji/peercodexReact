...
import { AP_SUBJECTS, GRADE_LEVELS, SCHOOLS } from '../constants'
...
const Profile = ({ standalone = true }) => {
    ...
    const profileContent = (
        <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex justify-between items-center mb-6">
                            ...
                            <div className="w-32 bg-gray-200 rounded-full h-2 mr-3">
                                <div
                                    className={`h-2 rounded-full transition-all duration-300 ${
                                        completionPercentage === 100 ? 'bg-green-500' : completionPercentage >= 80 ? 'bg-blue-500' : completionPercentage >= 60 ? 'bg-yellow-500' : 'bg-red-500'
                                    }`}
                ...
                {!isEditing ? (
                    <button onClick={() => setIsEditing(true)} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
                        Edit Profile
                    </button>
                ) : (
                    <div className="space-x-2">
                        <button onClick={handleCancel} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:border-gray-400 transition-colors">
                            Cancel
                        </button>
                        <button onClick={handleSave} disabled={isLoading} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-50">
                        ...
                        {isEditing ? (
                            <div>
                                <select
                                    ...
                                    className={`w-full px-3 py-2 border rounded-lg focus:ring-indigo-500 focus:border-indigo-500 ${errors.school ? 'border-red-500' : 'border-gray-300'}`}
                                >
                                ...
                                {showSchoolRequest && (
                                    <div className="mt-4 p-4 border border-indigo-200 rounded-lg bg-indigo-50">
                                        ...
                                        <p className="text-sm text-gray-600 mb-3">Can't find your school? We'll add it to our list after review.</p>
                                        <input
                                            type="text"
                        ...
                        {isEditing ? (
                            <div>
                                <select
                                    ...
                                    onChange={handleInputChange}
                                    className={`w-full px-3 py-2 border rounded-lg focus:ring-indigo-500 focus:border-indigo-500 ${errors.grade ? 'border-red-500' : 'border-gray-300'}`}
                                >
                        ...
                        {isEditing ? (
                            <div>
                                <input
                                    ...
                                    onChange={handleInputChange}
                                    className={`w-full px-3 py-2 border rounded-lg focus:ring-indigo-500 focus:border-indigo-500 ${errors.phoneNumber ? 'border-red-500' : 'border-gray-300'}`}
                            ...
                            <div>
                                ...
                                {userProfile.phoneNumber && <p className="text-xs text-gray-500 mt-1">Shared with mentoring connections</p>}
                            </div>
                        )}
                    </div>
                    <div>
                        ...
                        {isEditing ? (
                            ...
                            <div>
                                ...
                                {userProfile.discord && <p className="text-xs text-gray-500 mt-1">Shared with mentoring connections</p>}
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="mb-8">
                ...
                <p className="text-gray-600 mb-4">Select subjects where you feel confident helping other students.</p>
                {isEditing ? (
    ...
    return standalone ? (
        <div className="min-h-screen bg-gray-50 py-8">
            <div className="max-w-4xl mx-auto px-4">{profileContent}</div>
        </div>
    ) : (
        profileContent
    )
