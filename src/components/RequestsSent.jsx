                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        {req.status === 'accepted' && profile && (
                                            <div className="flex gap-2">
                                                ...
                                                {pointBalance > 0 && (
                                                    <button className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-medium" onClick={() => { setSelectedMentor(req.requesteeui); setShowPointsModal(true); }}>Award Points</button>
                                                )}
                                            </div>
                                        )}
                                        {/* Award Points Modal */}
                                        {showPointsModal && selectedMentor && (
                                            <div className="fixed inset-0 bg-black/10  flex items-center justify-center z-50">
                                                <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                                                    <button className="absolute top-3 right-3 text-gray-500 hover:text-gray-700" onClick={() => setShowPointsModal(false)}>&times;</button>
                                                    <h2 className="text-xl font-bold text-indigo-700 mb-4">Award Points</h2>
                                                    <div className="mb-2 text-gray-700">Your current balance: <span className="font-bold text-indigo-600">{pointBalance}</span></div>
                                                    <div className="mb-4">Select points to award:</div>
                                                    <div className="flex gap-2 mb-6 flex-wrap">
                                                        {[5, 10, 15, 20, 25, 30, 35, 40, 45, 50].filter(p => p <= pointBalance).map(p => (
                                                            <button key={p} className="px-4 py-2 bg-indigo-100 text-indigo-700 rounded-lg font-semibold hover:bg-indigo-200" disabled={awardLoading} onClick={() => handleAwardPoints(selectedMentor, p)}>{p}</button>
                                                        ))}
                                                    </div>
