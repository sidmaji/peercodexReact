...
const RequestsReceived = () => {
    ...
    useEffect(() => {
        const fetchRequests = async () => {
            setLoading(true)
            try {
                const q = query(collection(db, 'requests'), where('requesteeui', '==', currentUser?.uid || ''))
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        {req.status === 'pending' && (
                                            <div className="flex gap-2">
                                                ...
                                                <button className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 text-sm font-medium" onClick={() => handleAction(req, 'rejected')}>
                                                    Reject
                                                </button>
                                            </div>
                                        )}
                    ...
                    <div className="bg-white rounded-xl shadow-xl p-8 max-w-md w-full relative">
                        ...
                        <div className="flex justify-end gap-2">
                            <button
                                ...
                                onClick={() => handleAction(selectedRequest, 'accepted')}
                            >
                                {actionLoading ? 'Accepting...' : 'Accept'}
                            </button>
                            <button className="px-6 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 font-medium" onClick={() => setShowDisclaimer(false)}>
                                Cancel
                            </button>
                        </div>
                    </div>
